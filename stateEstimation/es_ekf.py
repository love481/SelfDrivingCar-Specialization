...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    q_hat=Quaternion(euler=delta_xk[6:]).quat_mult_right( q_check)
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    p_est[k] = p_est[k-1]+delta_t*v_est[k-1]+((delta_t**2)/2)*(C_ns.dot(imu_f.data[k-1])+g)
    ...
    F_k_1[3:6,6:9] = -(C_ns@skew_symmetric(imu_f.data[k-1].reshape((3,1)))) * delta_t
    Q_k_1=(delta_t**2)*np.diag([var_imu_f,var_imu_f ,var_imu_f ,var_imu_w ,var_imu_w ,var_imu_w ])
    # 2. Propagate uncertainty
    p_cov[k]=F_k_1.dot(p_cov[k-1]).dot(F_k_1.T)+l_jac.dot(Q_k_1).dot(l_jac.T)
